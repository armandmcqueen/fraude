...
│   │   ├── SummaryService.ts     # LLM-powered message summarization
...
interface Message {
  ...
  // Summary fields (assistant messages only, generated by SummaryService)
...
3. MultiPersonaChatSession:
   ...
   - **Sequential mode**: For each persona in order:
     ...
     - After streaming: generates summary if content > 500 chars
   - **Parallel mode**:
     - Creates all placeholder messages upfront
     - Captures history snapshot
     - Fires all LLM calls simultaneously
     - Summarization runs in parallel after each stream completes
   - Generates title if first exchange
   - Saves conversation (includes summaries)
4. useChat receives events (including summaryStart/summaryEnd), updates React state
5. UI re-renders showing persona names above each response
6. Messages with summaries show summary by default with toggle to view full content
...
| ID | Purpose |
|----|---------|
| claude-haiku-4-5-20251001 | Title generation, message summarization |
...
- `SummaryService` - message summarization
- `ChatSession` - full conversation flow with events, title generation, storage
- Multi-turn conversations with context
- `MultiPersonaChatSession` - sequential and parallel persona execution, summarization
